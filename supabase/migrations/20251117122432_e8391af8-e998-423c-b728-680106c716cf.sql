-- Update reset_generated_data function to include bpmn_node_index
...
BEGIN
  ...
  -- Check if bpmn_node_index table exists and count
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'bpmn_node_index') THEN
    EXECUTE 'SELECT COUNT(*) FROM bpmn_node_index' INTO node_index_count;
  ELSE
    node_index_count := 0;
  END IF;
  ...
  -- Truncate bpmn_node_index if it exists
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'bpmn_node_index') THEN
    EXECUTE 'TRUNCATE TABLE bpmn_node_index RESTART IDENTITY';
  END IF;
  ...
  result := jsonb_build_object(
    ...
    'deleted', jsonb_build_object(
      ...
      'bpmn_node_index', node_index_count
    ),
