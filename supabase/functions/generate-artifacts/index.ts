...
// Jira naming is now handled by handleBuildHierarchy in the client
// No longer generating jira_name here to avoid conflicts
...
serve(async (req) => {
  ...
  try {
    ...
    // 9. Initialize jira_type for testable nodes (if missing)
    // NOTE: jira_name is NOT generated here anymore.
    // Jira-namn genereras istället av handleBuildHierarchy() som använder ProcessTree
    // och kan bygga korrekta fullständiga paths. Här sätter vi bara jira_type om det saknas.
    // 
    ...
    for (const element of testableElements) {
      ...
      // Only set jira_type if it's missing (don't overwrite existing)
      // Don't set jira_name here - it should be generated by handleBuildHierarchy
    ...
    if (jiraTypeMappings.length > 0) {
      ...
      if (jiraTypeError) {
        console.error('Jira type initialization error:', jiraTypeError);
      } else {
        console.log(`Initialized jira_type for ${jiraTypeMappings.length} nodes (jira_name should be set via handleBuildHierarchy)`);
      }
    }
...
// NOTE: Hierarchy building functions removed.
// Jira-namn genereras nu endast av handleBuildHierarchy() i klienten som använder ProcessTree.
// Edge Function sätter bara jira_type om det saknas, inte jira_name.
