...
describe('renderBusinessRuleDoc', () => {
  ...
  it('renders business rule doc with expected section headings in order', () => {
    ...
    const prereqIndex = html.indexOf('Förutsättningar &amp; kontext');
    ...
    const businessIndex = html.indexOf('Affärsregler &amp; policystöd');
    ...
    expect(prereqIndex).toBeGreaterThan(summaryIndex);
    expect(inputsIndex).toBeGreaterThan(prereqIndex);
    expect(logicIndex).toBeGreaterThan(inputsIndex);
    expect(outputIndex).toBeGreaterThan(logicIndex);
    expect(businessIndex).toBeGreaterThan(outputIndex);
    expect(scenariosIndex).toBeGreaterThan(businessIndex);
