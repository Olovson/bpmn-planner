# Analys: När exakt sker redirecten till /auth?

## Problem

URL:en blir `http://localhost:8080/files#/auth` - vi är på `/files`-sidan, men hash-router navigerar till `#/auth` inuti sidan.
...
export const ProtectedRoute = ({ children }: ProtectedRouteProps) => {
  ...
  if (!authenticated) {
    return <Navigate to="/auth" replace />;  // <-- Detta blir #/auth i HashRouter
  }
...
1. **När vi navigerar till `/files` efter login:**
   ...
   - Då renderar den `<Navigate to="/auth" replace />` vilket i HashRouter blir `#/auth`
